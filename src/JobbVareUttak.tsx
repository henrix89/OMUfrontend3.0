...
export default function JobbVareUttak({ mobilvisning, verkstedId }: { mobilvisning: boolean; verkstedId: string }) {
      ...
      try {
        ...
        varer.forEach((item: Vare) => {
          const key = item.strekkode && item.strekkode !== "0"
            ? item.strekkode.toLowerCase().trim()
            : item.varenummer.toLowerCase().trim();
          record[key] = item;
        });
        setVaredata(record);
      } catch (err) {
    ...
    try {
      ...
      if (videoRef.current) {
        ...
        await videoRef.current.play().catch(() =>
          setPopup("🎥 Trykk for å aktivere kamera.")
        );
        ...
        codeReader.decodeFromVideoDevice(null, videoRef.current, (result, err) => {
          ...
          if (err && !(err instanceof NotFoundException)) {
            console.warn("Feil ved skanning:", err);
          }
        });
      }
    } catch (err) {
  ...
  {pdfTittel ? `Arbeidsordre: ${pdfTittel}` : "Ingen arbeidsordre valgt"}
      ...
      <div className="flex gap-2 mb-4">
        ...
        <button onClick={startScanning} className="bg-blue-600 text-white px-4 py-2 rounded-lg"><Camera className="h-5 w-5" /></button>
      </div>
      ...
      <button
        className="w-full bg-gray-200 py-2 rounded-lg mb-4"
    ...
    {varer.map((v) => (
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => oppdaterHandlekurv(varer.map(x => x.kode === v.kode ? { ...v, antall: Math.max(1, v.antall - 1), totalEksMva: v.pris * (v.antall - 1), totalInkMva: v.pris * (v.antall - 1) * MVA_SATS } : x))}
          ...
          <input
            ...
            onChange={(e) => {
              ...
              oppdaterHandlekurv(varer.map(x => x.kode === v.kode ? { ...v, antall: nyttAntall, totalEksMva: v.pris * nyttAntall, totalInkMva: v.pris * nyttAntall * MVA_SATS } : x));
            }}
            className="w-16 text-center border rounded"
          />
          <button
            onClick={() => oppdaterHandlekurv(varer.map(x => x.kode === v.kode ? { ...v, antall: v.antall + 1, totalEksMva: v.pris * (v.antall + 1), totalInkMva: v.pris * (v.antall + 1) * MVA_SATS } : x))}
            className="px-2 py-1 border rounded"
          >+</button>
      ...
      {scanning && (
        <video ref={videoRef} autoPlay playsInline muted className="w-full mt-4 rounded shadow" />
      )}

      {popup && <p className="text-center text-sm text-red-500 mt-2">{popup}</p>}

      {visJobbModal && (
        <JobblisteModal onSelectJob={handleSelectJob} onClose={() => setVisJobbModal(false)} />
      )}
      ...
      {visManuellModal && redigeringsVare && (
        <ManuellKorrigeringModal
          ...
          onSave={async (v) => {
            const nyListe = varer.map((x) => (x.kode === v.kode ? v : x));
